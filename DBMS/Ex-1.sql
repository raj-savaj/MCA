...
CREATE TABLE class(cname varchar2(20),meet_at varchar2(10),room varchar2(10),fid numer(5) REFERENCES faculty(fid));
CREATE TABLE enrolled(snum NUMBER(5) REFERENCES student20(snum),cname varchar2(20), PRIMARY KEY(snum,cname));
...
SELECT sname FROM student WHERE snum IN(SELECT e1.snum FROM enrolled e1,enrolled e2,class c1,class c2 WHERE e1.snum=e2.snum AND e1.cname<>e2.cname AND e1.cname=c1.cname AND e2.cname=c2.cname AND c1.meet_at=c2.meet_at);
SELECT sname FROM student WHERE snum IN(SELECT snum FROM enrolled WHERE cname IN (SELECT cname FROM class WHERE meet_at IN (SELECT  meet_at FROM class GROUP BY meet_at HAVING COUNT(*) >= 2)) GROUP BY snum HAVING COUNT(*) >=2);
